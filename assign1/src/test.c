...
//	printf("%s",printCalendar(c));
	...
	printf("%s\n",s);
	printf("%s\n",e);
	Calendar * cal=JSONtoCalendar("{\"version\":2,\"prodID\":\"thi sjdfnrweidferj3l4kr4k4rkl2./\"}");
	Event*eventa=JSONtoEvent("{\"UID\":\"testing thus fyjnbgr\"}");
	printf("%f,%s\n",cal->version,cal->prodID);
	printf("%s\n",eventa->UID);
	freeList(cal->events);
	freeList(cal->properties);
	free(cal);
	free(eventa->alarms);
	free(eventa->properties);
	free(eventa);
	deleteCalendar(c);
	free(s);
	free(e);
